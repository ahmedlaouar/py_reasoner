...
def is_strictly_preferred(cursor, pos_mat, w_assertion_1, w_assertion_2) -> bool:
    # a test if assertion_1 is strictly preferred to assertion_2
    weight_1 = w_assertion_1.get_assertion_weight()
    weight_2 = w_assertion_2.get_assertion_weight()
...
def check_all_dominance(cursor, pos, conflicts, supports):
    ...
    for conflict in conflicts:
        ...
        for support in supports:
            ...
            if is_strictly_preferred(cursor, pos, support, conflict[0]) or is_strictly_preferred(cursor, pos, support, conflict[1]):
