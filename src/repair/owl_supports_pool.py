...
def compute_all_supports(assertions, ontology_path: str, cursor: Cursor, pos_dict, data_path):
    ...
    supports = {}
    arguments = [(assertion, cqueries[assertion], pos_dict, data_path) for assertion in cqueries.keys()]
    print("Before going into the pool!")
    with Pool(processes=100) as pool:
        results = pool.map(supports_helper, arguments)
    print("Out of the pool")
    for assertion, supps in results:
        supports[assertion] = supps
    return supports

def compute_all_supports_enhanced(assertions, ontology_path: str, cursor: Cursor, pos_dict, data_path):
...
def supports_helper(args):
    assertion, queries, pos_dict, data_path = args
    supports = set()
    try:
        ...
        sql_queries = []       
        for query in queries:
            sql_queries.append((generate_sql_query(query)))
        rows = []
        
        for sql_query,table_name in sql_queries:
            cursor.execute(sql_query)
            rows = cursor.fetchall()
            for row in rows:
                if len(row) == 3:
                    element = (table_name, row[1], None, row[2])
                if len(row) == 4:
                    element = (table_name, row[1], row[2], row[3])
            
                if not any(dominates(pos_dict, [support], [element]) for support in supports):
                    to_remove = {support for support in supports if dominates(pos_dict, [element], [support])}
                    if to_remove:
                        supports = supports - to_remove
                    supports.add(element)
        
        cursor.close()
        conn.close()
    except sqlite3.OperationalError as e:
        print(f"Error: {e}.")
    return assertion, supports
