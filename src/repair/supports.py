...
def supports_deduction(find_assertion, positive_axioms, cursor):
    supports = []
...
def compute_supports(target_assertion, positive_axioms, cursor):
    ...
    row = cursor.fetchone()
    if row is not None:
        if row[3] == 'None' or row[3] is None:
            new_assertion = w_assertion(row[1], row[2], weight=row[4])
        else:
            new_assertion = w_assertion(row[1], row[2], row[3], weight=row[4])
        supports.append(new_assertion)
